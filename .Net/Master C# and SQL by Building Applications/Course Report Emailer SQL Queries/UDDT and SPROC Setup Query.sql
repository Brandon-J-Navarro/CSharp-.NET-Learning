...
CREATE PROCEDURE [dbo].[Course_GetList] AS 
    SELECT
        CourseId,
        CourseCode,
        [Description]
...
CREATE PROCEDURE [dbo].[Student_GetList] AS 
    SELECT
        StudentId,
        FirstName,
        LastName
...
CREATE PROCEDURE [dbo].[Enrollments_GetList] AS 
    SELECT
        EnrollmentId,
        StudentId,
        CourseId
...
CREATE TYPE [dbo].[Enrollment_Type] AS TABLE (
...
CREATE PROCEDURE [dbo].[Enrollment_Upsert]
    @EnrollmentType [Enrollment_Type] READONLY,
    @UserId VARCHAR(50)
AS
