    ...
    IViewModel
{
    ...
    public MainViewModel(IRepository<User> userRepository, IRepository<TodoTask> todoRepository)
    {
        WeakReferenceMessenger.Default.Register<TodoSavedMessage>(this,(sender, message) =>
        {
            ...
            if(item.IsCompleted)
            {
                var exisiting = Unfinished.FirstOrDefault(i => i.Id == item.Id);

                if(exisiting is not null)
                {
                    Unfinished.Remove(exisiting);
                }
                ReplaceOrAdd(Completed, item);
            }
            else
            {
                var exisiting = Completed.FirstOrDefault(i => i.Id == item.Id);

                if (exisiting is not null)
                {
                    Completed.Remove(exisiting);
                }
    ...
    public async Task InitializeAsync()
    {
        ...
        App.CurrentUser = await userRepository.FindByAsync("Brandon");

        var items = await todoRepository.AllAsync();

        var itemsDue = items.Count(i => i.DueDate.ToLocalTime() > DateTimeOffset.Now);

        StatusText = $"Welcom {App.CurrentUser.Name}! " + $"You have {itemsDue} items passed due date.";
